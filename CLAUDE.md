...
│   ├── analyze.rb        # db/health.db → JSON stdout
│   ├── mcp_server.rb     # MCPサーバー（Claudeのアドホック分析用）
...
# 日付範囲指定
bundle exec ruby scripts/analyze.rb 20200101 20261231
...
## MCPサーバー（Claudeアドホック分析）
`.claude/settings.json` の `mcpServers` セクションで設定済み。
Claude Codeを再起動するとMCPサーバーが有効になり、`query_health` / `get_db_info` ツールが使えるようになる。
