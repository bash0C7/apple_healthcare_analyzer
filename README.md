...
- **Claude Code スキル** — `/apple-health-analyze` でサブエージェントが自動分析・レポート生成
- **MCP サーバー** — Claude Code / Claude Desktop からアドホックSQLクエリ可能
...
### 4. Claude Code でレポート生成

```
/init-health-db                            # DB初期化（初回）
/apple-health-analyze                      # 直近90日のレポート
/apple-health-analyze 30                   # 直近30日
/apple-health-analyze 20260101 20260329    # 日付範囲指定
```

サブエージェントが自動的に分析し、以下のセクションを日本語でレポートします：
- 健康状態サマリー（各指標の現状・異常値フラグ）
- 身体スコア推移（直近14日）
- マルチホライズンフォーキャスト（1週〜6ヶ月）
- アドバイス（データから直接導出）
