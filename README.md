...
- **MCP 統合** — chiebukuro-mcp 経由で Claude Code / Claude Desktop からアドホックSQLクエリ・自然言語分析が可能
...
Claude Code から `/apple-health-care-init-db` コマンドを実行するとサブエージェントが自動で実行します。
手動で実行する場合：
...
Claude Code を使っている場合は `/apple-health-care-update-db` コマンドが上記を自動で実行します：

```
/apple-health-care-update-db                                    # data/export.xml が差し替え済みの場合
...
# アドホッククエリ（SQL直接）
chiebukuro_query_health ツールで以下のSQLを実行して：
SELECT date, body_battery, hrv, resting_hr FROM daily_summary
WHERE date >= '2026-01-01' ORDER BY date;
```

## MCP 統合（chiebukuro-mcp 経由）

health.db の読み取りは chiebukuro-mcp（外部 MCP サーバー）経由で行います。
自前の `scripts/mcp_server.rb` は廃止しました。

### chiebukuro-mcp への追加手順

1. `/Users/bash/dev/src/github.com/bash0C7/ruby-knowledge-db/config/chiebukuro.json` の
   `databases` オブジェクトに以下を追加:

```json
"health": {
  "path": "/Users/bash/dev/src/github.com/bash0C7/apple_healthcare_analyzer/db/health.db",
  "description": "Apple HealthKit データ分析DB。日次サマリー・心拍・歩数・睡眠・body_battery を収録。"
}
```

> パスはご自身の環境に合わせて変更してください。

2. Claude Desktop / Claude Code を再起動

3. `chiebukuro_query_health` ツールが使えることを確認:
   ```sql
   SELECT date, step_count, body_battery FROM daily_summary ORDER BY date DESC LIMIT 5
   ```
...
├── .claude/
│   ├── commands/
│   │   ├── apple-health-care-init-db.md     # /apple-health-care-init-db コマンド定義
│   │   └── apple-health-care-update-db.md   # /apple-health-care-update-db コマンド定義
